...
def set_loader(opt, model):
    dataset = datasets.__dict__[opt.data.dataset]
    train_dataset = dataset(seed=opt.seed, split='train', **opt.data)
    val_dataset = dataset(seed=opt.seed, split='val', **opt.data)
    test_dataset = dataset(seed=opt.seed, split='test', **opt.data)
...
def validate(loader, regressor, return_r2_score=False):
    regressor.eval()
...
def train_regressor(opt=None):
    ...
    test_loss, test_r2_score = validate(loader=test_loader, regressor=regressor, return_r2_score=True)
...
if __name__ == '__main__':
    opt = parse_regressor_option()
    set_logging(opt)
    set_seed(opt.seed) # Fixing random seed    
    train_regressor(opt)
