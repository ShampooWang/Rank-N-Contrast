...
def set_loader(opt):
    ...
    train_dataset = dataset(data_folder=opt.data_folder, aug=opt.aug, split='train', seed=opt.seed)
    val_dataset = dataset(data_folder=opt.data_folder, aug=opt.aug, split='val', seed=opt.seed)
    test_dataset = dataset(data_folder=opt.data_folder, aug=opt.aug, split='test', seed=opt.seed)
...
def train(train_loader, model, regressor, criterion, optimizer, epoch, opt):
    ...
    for idx, returned_items in enumerate(train_loader):
        ...
        bsz = labels.shape[0]
        # if idx == 0: print(labels)

        with torch.no_grad():
            features = model(images) 
            # features = features / (5 * math.sqrt(features.shape[1]))
...
def validate(val_loader, model, regressor, return_r2_score=False):
    model.eval()
    regressor.eval()
    ...
    if return_r2_score:
        return losses.avg, r2metric.compute()
    else:
        return losses.avg
...
def main():
    ...
    test_loss, test_r2_score = validate(test_loader, model, regressor, return_r2_score=True)
